  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      <div className="flex flex-col items-center prevent-select">
        ...
        <div
          className="grid grid-cols-6 gap-1 sm:gap-2 mb-6 justify-center relative touch-none"
          ref={gridRef}
            ...
            row.map((letter, colIndex) => (
              ...
              >
                <Button
                  ...
                  variant="ghost"
                  className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full text-lg sm:text-2xl p-0 flex items-center justify-center border-0 hover:bg-parent hover:text-primary-background z-10
          ...
          {[1, 2, 3].map((hintNumber) => (
            <HoverCard key={hintNumber}>
              <HoverCardTrigger className="mx-2 mb-2 sm:mb-0" asChild>
                <Button variant="outline">Hint {hintNumber}</Button>
              </HoverCardTrigger>
              <HoverCardContent className="w-80">
                {previousHints[hintNumber - 1] ? (
                  <p>{previousHints[hintNumber - 1]}</p>
                ) : (
                  <p>Use the "Use Hint" button to reveal this hint.</p>
                )}
              </HoverCardContent>
            </HoverCard>
          ))}
